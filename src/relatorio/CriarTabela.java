...
public class CriarTabela{
    ...
    public PdfPTable resultadoPontos(List<Equipe> listaEquipes) throws DocumentException {      
        float [] colunas = new float[]{0.5f, 2.5f, 1.0f, 1.0f, 1.0f, 2.0f, 2.0f}; 
        ...
        PdfPCell cell = new PdfPCell(new Phrase("PONTUAÇÃO TOTAL", font));
        ...
        tabela.addCell(criaCabecalho("Relatório"));
        tabela.addCell(criaCabecalho("Calouro"));
        tabela.addCell(criaCabecalho("Penalidade"));
        tabela.addCell(criaCabecalho("Classificatórias"));
        tabela.addCell(criaCabecalho("Final"));
        ...
        for (Equipe equipe : listaEquipes) {
            ...
            tabela.addCell(criaCelula(String.valueOf(round(equipe.getRelatorio(), 3))));
            tabela.addCell(criaCelula(
                    equipe.isCalouro() ? "SIM": "NÃO"
                    ));
            tabela.addCell(criaCelula(String.valueOf(round(equipe.getPenalidade(), 3))));
            tabela.addCell(criaCelula(String.valueOf(round(equipe.getClassificatoriaPontos(), 3))));
            tabela.addCell(criaCelula(String.valueOf(
                    equipe.getFinalPontos() == 0 ? "-----": round(equipe.getFinalPontos(), 3) )));
    ...
    public PdfPTable resultadoTempo(List<Equipe> listaEquipes, int bateria) throws DocumentException {
        ...
        if(bateria != Bateria.FINAL.getValue()){
            if (sprint) cell = new PdfPCell(new Phrase("SPRINT", font));
            else cell = new PdfPCell(new Phrase("TEMPO - " + (bateria + 1) + "° BATERIA", font));
        }
        else {
            ...
            if(bateria == Bateria.FINAL.getValue() && equipe.isClassificada()){
                ...
                for(int trecho_aux = 0; trecho_aux < trechos; trecho_aux++){
                    tabela.addCell(criaCelula(equipe.getTempoTrecho(bateria, trecho_aux) == Parametro.PENALIDADE.getValue() ?
                        "-----" : String.valueOf(round(equipe.getTempoTrecho(bateria, trecho_aux),3) + " s")));
                }
                tabela.addCell(criaCelula( equipe.getTempoTotal(bateria) == Parametro.PENALIDADE.getValue() ? 
                    "-----" : String.valueOf(round(equipe.getTempoTotal(bateria), 3)  + " s")));
                estado = !estado;
                colocacao++;               
            }
            else if(sprint){
                ...
                tabela.addCell(criaCelulaNomeEquipe(converteEncode(equipe.getNome())));
                tabela.addCell(criaCelula( equipe.getTempoTotal(bateria) == Parametro.PENALIDADE.getValue() ? 
                    "-----" : String.valueOf(round(equipe.getTempoSprint(), 3)  + " s")));
                estado = !estado;
                colocacao++;        
            }
